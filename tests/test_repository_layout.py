...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "dk"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_denmark_namespace_present() -> None:
    """Denmark is unitary: the only jurisdiction directory is dk/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"dk"}, f"unexpected jurisdiction dirs: {names - {'dk'}}"


def test_dk_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "dk" / marker).is_dir(), f"missing dk/{marker}"
...
def test_oracle_index_is_dk_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "dk"


def test_source_map_is_dk_scoped() -> None:
    payload = json.loads(
